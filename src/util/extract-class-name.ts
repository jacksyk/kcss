...
function extractClassNames(text: string, token: string = "styles", suffix: string = ".module.less"): string[] {
    ...
    const curPath = currentlySelectedFilePath.replace(/\.tsx$/, suffix) // 当前目录同级less文件的路径

    // 确保文件存在
    if (!fs.existsSync(curPath)) {
        ensureFileSync(curPath)
    }
